...
object YamlParser {
    ...
    fun parse(yamlString: String, albums: List<Album>): List<AutomationRule> {
        ...
        return rulesList.mapIndexed { index, map ->
            ...
            if (type != "Time" && type != "Location" && type != "Interval" && type != "Gesture") {
                throw Exception("Rule '$name' has invalid type '$type'. Must be 'Time', 'Location', 'Interval' or 'Gesture'")
            }
            ...
            if (logic != "NONE" && logic != "AND" && logic != "OR" && logic != "XOR") {
                throw Exception("Rule '$name' has invalid logic '$logic'. Must be 'NONE', 'AND', 'OR', or 'XOR'")
            }
            ...
            if (type == "Gesture" && gestureType != "DoubleTap" && gestureType != "TwoFingerDoubleTap" && gestureType != "ThreeFingerTripleTap") {
                throw Exception("Gesture rule '$name' has invalid gesture '$gestureType'. Must be 'DoubleTap', 'TwoFingerDoubleTap' or 'ThreeFingerTripleTap'")
            }
            
            val actionType = map["action"] as? String ?: throw Exception("Rule '$name' is missing field 'action'")
            if (actionType != "NextWallpaper" && actionType != "SwitchAlbum") {
                throw Exception("Rule '$name' has invalid action '$actionType'. Must be 'NextWallpaper' or 'SwitchAlbum'")
            }
